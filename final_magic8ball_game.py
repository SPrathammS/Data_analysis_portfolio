...
import random
from colorama import Fore, Style  #* we have imported 'colorama' and 'random' library and used their functions
...
def game_function():    #* we are defining a function that will ask a question, that generates a random number and chooses an item from the relevant list according to set condition.
 ...
 #print(rdm_num)  # to check the number for reference
#* here are two lists, one good and bad, from which the program will randomly select depending on the number generated
 # A list of common good responses for a magic 8ball
 good_response = ["It is certain", 
                 "It is decidedly so",
                 "Without a doubt",
                 "Yes definitely",
                 "You may rely on it"]
 # A list of common bad responses for a magic 8ball
